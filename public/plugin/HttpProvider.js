...
var ExtState = Ext.decode('<?#=json_encode($VALUES)?>'); 
...
Ext.extend(Ext.state.HttpProvider, Ext.state.Provider, {
    ...
    readValues : function(){
        ...
        for (var name in state)
        {
            if(name!='remove')
            {
                state[name] = this.decodeValue(state[name]);
            }
        }        
        return state;
    },
    ...
    setValue : function(name, value){    	
        ...
        conn.request({
            url: this.url,
            params: {cmd: 'jsonUserColumnSelection', 'gridId': name, 'columdata': this.encodeValue(value) }
        });
    },
    // private

...
require_once 'models/app_university/user/UserActivityDBAccess.php';
...
class UserController extends Zend_Controller_Action {
    ...
    public function init() {
        ...
        $this->DB_USER_MEMBER = UserMemberDBAccess::getInstance();
        ...
        if ($this->_getParam('objectId')) {
            ...
            $this->objectData = $this->DB_USER_MEMBER->getUserDataFromId($this->objectId);
        }
    ...
    public function jsonloadAction() {
        ...
        switch ($this->REQUEST->getPost('cmd')) {
            ...
            case "loadObject":
                $jsondata = $this->DB_USER_MEMBER->loadUserFromId($this->REQUEST->getPost('objectId'));
                break;

            case "allUsers":
                $jsondata = $this->DB_USER_MEMBER->allUsers($this->REQUEST->getPost());
                break;

            case "sessionExpired":
                $jsondata = $this->DB_USER_MEMBER->sessionExpired($this->REQUEST->getPost());
                break;
    ...
    public function jsonsaveAction() {
        ...
        switch ($this->REQUEST->getPost('cmd')) {
            ...
            case "updateObject":
                $jsondata = $this->DB_USER_MEMBER->updateUser($this->REQUEST->getPost());
                break;

            case "addObject":
                $jsondata = $this->DB_USER_MEMBER->addUser($this->REQUEST->getPost());
                break;

            case "removeObject":
                $jsondata = $this->DB_USER_MEMBER->removeObject($this->REQUEST->getPost());
                break;

            case "releaseObject":
                $jsondata = $this->DB_USER_MEMBER->releaseObject($this->REQUEST->getPost());
                break;
        }

...
class SQLEvaluationImport {
    ...
    public static function importScoreAssignment($stdClass)
    {
        ...
        if ($EXCEL_DATA->sheets)
        {
            ...
            {
                $this->actionImportScoreAssignment($EXCEL_DATA->sheets[$i], $stdClass);
            }
        }
    }

    public static function actionImportScoreAssignment($sheet, $stdClass)
    {
        $STUDENT_DATA = self::getListStudents($stdClass);

        for ($i = 0; $i <= $sheet['numRows']; $i++)
        {
            $date = isset($sheet['cells'][$i + 3][2]) ? $sheet['cells'][$i + 3][2] : "";

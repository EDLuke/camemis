...
namespace CAMEMIS\CAMEMISMySQLDump;

use Exception;
use PDO;
use PDOException;

class CAMEMISMySQLDump {
    ...
    /**
     * Constructor of CAMEMISMySQLDump. Note that in the case of an SQLite database
     ...
     * @return null
     */
    public function __construct($db = '', $user = '', $pass = '', $host = 'localhost', $type = "mysql", $settings = null, $pdoOptions = array(PDO::ATTR_PERSISTENT => true,
        ...
        $this->_settings = CAMEMISMySQLDump::array_replace_recursive($defaultSettings, $settings);
    }
    ...
    public static function array_replace_recursive($array1, $array2) {
        if (function_exists('array_replace_recursive')) {
            ...
            foreach ($array2 as $key => $value) {
                if (is_array($value)) {
                    $array1[$key] = CAMEMISMySQLDump::array_replace_recursive($array1[$key], $value);
                } else {
                    $array1[$key] = $value;
                }
            }
            ...
            switch ($this->_dbType) {
                ...
                case 'mysql': case 'pgsql': case 'dblib':
                    $this->_dbHandler = new PDO(
                            $this->_dbType . ":host=" .
                            $this->host . ";dbname=" . $this->db, $this->user, $this->pass, $this->_pdoOptions
                    );
    ...
    private function listValues($tablename) {
        ...
        foreach ($this->_dbHandler->query($stmt, PDO::FETCH_NUM) as $r) {
            ...
            if (($lineSize > CAMEMISMySQLDump::MAXLINESIZE) ||

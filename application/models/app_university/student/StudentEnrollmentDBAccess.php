...
class StudentEnrollmentDBAccess extends StudentDBAccess {
	...
	public static function findAcademicByGradeSchoolyear($gradeId, $schoolyearId) {

		$academicObject = AcademicDBAccess::findGradeFromId($gradeId);

		if ($academicObject->EDUCATION_SYSTEM) {
			$SQL = self::dbAccess()->select();
			$SQL->from(array('A' => 't_grade'), array("GUID AS ACADEMIC_ID"));
			...
			$SQL = self::dbAccess()->select();
			$SQL->from(array('A' => 't_student_schoolyear'), array("CLASS AS ACADEMIC_ID"));

...
class StudentDBAccess {
    ...
    protected function addStudent2GradeClassSchoolyear($studentId, $academicObject)
    {
        ...
        {
            $facette = StudentAcademicDBAccess::findStudentClassGradeSchoolyear($studentId, $academicObject);
            if ($facette)
            {

                if (!$facette->CLASSIDS)
                {
                    $newIds = $academicObject->ID;
                }
                else
                {
                    ...
                    if ($Ids == 1)
                    {
                        $newIds = $facette->CLASSIDS . "," . $academicObject->ID;
                    }
                    else
                    {
                        array_push($Ids, $academicObject->ID);
                        $newIds = implode(",", array_unique($Ids));
                    }
                }
            }
    ...
    public function jsonAddStudent2GradeClassSchoolyear($params)
    {
        ...
        if ($SQLIds != "" && $academicObject)
        {
            ...
            if ($selectedStudents)
                foreach ($selectedStudents as $studentId)
                {
                    ...
                    {
                        $this->addStudent2GradeClassSchoolyear(
                                $studentId
                                , $academicObject);
    ...
    public static function jsonRemoveEnrolledStudentFromClass($params)
    {
        ...
        if ($academicObject)
        {
            ...
            if ($academicObject->ENROLLMENT_TYPE == 1)
            {
                $SQL .= " AND FIND_IN_SET($academicObject->ID,CLASSIDS)";
            }
            else
            {
    ...
    public function actionStudentGradeClassSchoolyear($params)
    {
        ...
        switch ($field)
        {
            ...
            case "FOURTH_ACADEMIC":
                ...
                if ($newValue)
                {
                    if (!StudentAcademicDBAccess::checkUsedStudentClassTermSchoolyear($studentId, $academicObject, $field))
                    {
                        $ACTION_VALUE = $academicObject->ID;
                        $SAVEDATA["" . $field . ""] = $ACTION_VALUE;
                    }
                }
                else
                {
                    $facette = StudentAcademicDBAccess::findStudentClassGradeSchoolyear($studentId, $academicObject);
                    if ($facette->$field == $academicObject->ID)
                    {
                        $SAVEDATA["" . $field . ""] = 0;
                    }
                }
    ...
    public static function checkStudentINGradeSchoolyear($studentId, $academicObject)
    {
        ...
        $CHECK_SQL->where("STUDENT_ID='" . $studentId . "'");

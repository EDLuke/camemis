...
class UserMemberDBAccess {
    ...
    public function getUserDataFromId($Id) {
        ...
        $facette = self::findUserFromId($Id);

        $data = array();

        if ($facette) {

            if (!$facette->LOGINNAME) {
                self::setMappingCode_LoginName($facette->ID);
            }

            $data["ID"] = $facette->ID;
            $data["LOGINNAME"] = setShowText($facette->LOGINNAME);
            $data["FIRSTNAME"] = setShowText($facette->FIRSTNAME);
            $data["LASTNAME"] = setShowText($facette->LASTNAME);
            $data["STATUS"] = $facette->STATUS;
            $data["ROLE"] = $facette->ROLE;
            $data["ADDITIONAL_ROLE"] = $facette->ADDITIONAL_ROLE;
            ...
            $data["ROLE"] = $facette->ROLE;
            $data["USER_ROLE"] = $facette->ROLE;
            $data["IS_PASSWORD"] = $facette->PASSWORD;
            $data["SUPERUSER"] = $facette->SUPERUSER;
            ...
            $data["REMOVE_STATUS"] = $this->checkRemoveUser($facette->ID);
        }
    ...
    public function releaseObject($params) {
        ...
        self::dbAccess()->update("t_members", $data, "ID='". $objectId ."'");

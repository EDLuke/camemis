                ...
                foreach ($result as $value) {
                    ...
                    if ($value->TAX_PERCENT && $value->TAX_FORMULAR) {
                        switch ($value->TAX_FORMULAR) {
                            case 1:
                                $AMOUNT_TAX = calculateTax($value->TRANSACTION_AMOUNT, $value->TAX_PERCENT);
                                ...
                                if($value->SCHOLARSHIP_INDEX == 100){
                                    $data[$i]["TRANSACTION_AMOUNT"] = "0 ". self::getCurrency();
                                    $data[$i]["TOTAL_AMOUNT"] = "0 ". self::getCurrency();
                                    $data[$i]["TOTAL_AMOUNT"] .= " (".displayNumberFormat($value->SCHOLARSHIP_INDEX)."%)";
                                    $data[$i]["AMOUNT_TAX"] = "0 " . self::getCurrency();
                                }else{ 
                                    $data[$i]["TOTAL_AMOUNT"] = calculateTaxPlus($value->TRANSACTION_AMOUNT, $value->TAX_PERCENT) . " " . self::getCurrency();
                                    if($value->SCHOLARSHIP_INDEX){
                                        $data[$i]["TOTAL_AMOUNT"] .= " (".displayNumberFormat($value->SCHOLARSHIP_INDEX)."%)";
                                    }
                                    $data[$i]["AMOUNT_TAX"] = calculateTax($value->TRANSACTION_AMOUNT, $value->TAX_PERCENT) . " " . self::getCurrency();
                                }
                            ...
                            case 2:
                                ...
                                $data[$i]["TAX_CATEGORY"] = $value->TAX_NAME;
                                $data[$i]["TAX_CATEGORY"] .= "<br>";
                                $data[$i]["TAX_CATEGORY"] .= " (-" . $value->TAX_PERCENT . "%)";
                        ...
                        $data[$i]["TOTAL_AMOUNT"] = displayNumberFormat($value->TRANSACTION_AMOUNT) . " " . self::getCurrency();
                        $data[$i]["AMOUNT_TAX"] = "---";
                    }

...
require_once 'models/app_university/extraclass/ExtraClassDBAccess.php';
...
class StudentExtraClassDBAccess extends ExtraClassDBAccess {
   ...
   public static function sqlStudentExtraClass($globalSearch, $extraClassId, $studentId) {
       ...
       /*if ($EXTRACLASS_OBJECT) {
            switch ($EXTRACLASS_OBJECT->OBJECT_TYPE) {
                case "TERM":
                    $SQL->where('A.TERM = ?', $extraClassId);
                    break;
                case "LEVEL":
                    $SQL->where('A.LEVEL = ?', $extraClassId);
                    break;
                case "CLASS":
                    $SQL->where('A.EXTRACLASS = ?', $extraClassId);
                    break;
    ...
    public static function jsonStudentExtraClass($params, $isJson = true) {
        ...
        $facette = self::findExtraClassFromId($extraClassId);
        if ($facette) {

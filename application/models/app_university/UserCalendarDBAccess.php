...
class UserCalendarDBAccess {
    ...
    public function jsonLoadUserCalendar() {
        ...
        switch (UserAuth::getUserType()) {
            ...
            case "STUDENT" :
                ...
                $evt_params['current_academicId'] = $USER['CURRENT_CLASS_ID'];
    ...
    public function sqlSchoolEvent($params) {
        ...
        if ($academicId || $academicId == 0 || $current_academicId) {
            if ($academicId || $academicId == 0)
                $this->SELECT->where('CLASS_ID = ?', $academicId);
            if ($current_academicId)
                $this->SELECT->orWhere('CLASS_ID = ?', $current_academicId);
        }
        if ($teacherId) {

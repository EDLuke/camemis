    ...
    public static function sqlAcademicAdditionalByStudent($studentId, $classId, $target = '') {
        ...
        switch (strtolower($target)) { //@Man
            case 'traditional':
                ...
                break;
            case 'credit':
                $SQL->joinLeft(array('C' => 't_student_schoolyear_subject'), 'B.GRADE=C.CAMPUS_ID AND B.SCHOOL_YEAR=C.SCHOOLYEAR_ID', array());
                $SQL->joinLeft(array('D' => 't_grade'), 'C.CAMPUS_ID=D.ID', array());
                if ($studentId)

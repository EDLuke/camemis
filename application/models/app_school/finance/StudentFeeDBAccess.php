...
class StudentFeeDBAccess extends FeeDBAccess {
    ...
    public static function setStudentFeePayment($params) {
        ...
        if (!$trainingId) {
            ...
            //$isScholarship = $studentAcademicObject ? $studentAcademicObject->SCHOLARSHIP : false;
        } else {
        ...
        if ($STUDENT_FEE && $FEE_OBJECT) {
            ...
            if ($scholarshipPercent) {
                ...
                if ($taxFormular == 1) {
                    ...
                    $amountNoTax = $AMOUNT_PAID + $AMOUNT_DISCOUNT;
                    $amountWithTax = $amountNoTax - $AMOUNT_TAX;
                    $totalFee = floatval($STUDENT_FEE->TOTAL_AMOUNT);

                    $AMOUNT_OWED = $totalFee - $amountWithTax;

                    //error_log("AMOUNT_OWED=AmountTotal-amountwithtax=".$totalFee."-".$amountWithTax."=".$AMOUNT_OWED."");
                } else {
        ...
        if ($result) {
            foreach ($result as $value) {
                ...
                if ($value->FEE_TYPE == "GENERAL_EDU") {
                    ...
                    $data[$i]["ACADEMIC"] = GENERAL_EDUCATION;
                }
                ...
                if ($value->FEE_TYPE == "COURSE") {
                    ...
                    $data[$i]["ACADEMIC"] = TRAINING_PROGRAMS;
                }
    ...
    public static function getSQLSearchCasshStudent($params) {
        ...
        /*
          if ($scholarship)
          $SQL->where("A.SCHOLARSHIP = '" . $scholarship . "'");
         */
        if ($paymentStatus)
    ...
    //
    //veasna
    public static function loadStudentAlert($params) {
        $result = self::getSQLSearchCasshStudent($params);
        $data = array();
        $i = 1;
        $checkID = "";
        ...
        if ($result) {
            foreach ($result as $value) {
                if ($checkID == $value->STUDENT_ID) {
                    continue;
                }
                $reciever[] = $i . ") " . $value->LASTNAME . " " . $value->FIRSTNAME;
                $checkID = $value->STUDENT_ID;
                ++$i;
            }
        }
        $data["SUBJECT"] = "Alert";
        $data["STR_RECIPIENT_NAME"] = implode("\n", $reciever);
        $data["CONTENT"] = "Please Come and Pay For Your School Fee!";
        $o = array(
            "success" => true
            , "data" => $data
        );
            ...
            foreach ($result as $value) {
                ...
                if ($studentPrePay && !$value->STUDENT_SERVICES) {
                    ...
                    switch ($value->FEE_TYPE) {
                        case "GENERAL_EDU":
                            $data[$i]["FEE_TYPE"] = GENERAL_EDUCATION;
                    ...
                    if ($value->CHOOSE_OPTION >= 1) {
                        ...
                        $CHOOSE_OPTION = "---";
                        //$data[$i]["TRANSACTION_AMOUNT"] = $feeOptionObject->TOTAL ? getNumberFormat($feeOptionObject->TOTAL) . " " . self::getCurrency() : "0 " . self::getCurrency(); 
                        $data[$i]["TRANSACTION_AMOUNT"] = $value->TOTAL_AMOUNT ? displayNumberFormat($value->TOTAL_AMOUNT) . " " . self::getCurrency() : "0 " . self::getCurrency();
                    }

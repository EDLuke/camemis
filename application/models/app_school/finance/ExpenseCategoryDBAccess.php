<?php
    ...
    class ExpenseCategoryDBAccess {
        ...
        public static function checkUsedExpenseCat($Id){
            ...
            if($checkChilde){
                foreach($checkChilde as $value){
                    ...
                    $SQL->where("EXPENSE_CATEGORY = '".$value->ID."'");    
                    $result= self::dbAccess()->fetchRow($SQL);
        ...
        public static function updateObject($params) {
            ...
            if ($objectId == "new") {
                ...
                if (!$errors) {
                    self::dbAccess()->insert('t_expense_category', $SAVEDATA);
                ...
                if (!$errors)
                    self::dbAccess()->update('t_expense_category', $SAVEDATA, $WHERE);
            }
        ...
        public static function sqlExpenseCategory($node) {

            $SQL = "SELECT * FROM t_expense_category";
            $SQL .= " WHERE 1=1";
            if (!$node)
                $SQL .= " AND PARENT=0";
            else
        ...
        public static function jsonTreeAllExpenseCategories($params) {

            $node = isset($params["node"]) ? addText($params["node"]) : 0;

            $data = array();
            $result = self::sqlExpenseCategory($node);
        ...
        public function allExpenseCategoryComboData($parent, $type = false) {

            $data = array();
            $result = self::sqlExpenseCategory($parent, $type);
        ...
        public static function findTaxByCategory($Id) {
            ...
            $SQL = "";
            $SQL .= "SELECT B.PERCENT AS PERCENT, B.FORMULAR AS FORMULAR";
            $SQL .= "FROM t_expense_category AS A";

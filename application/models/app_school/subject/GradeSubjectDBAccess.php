...
class GradeSubjectDBAccess extends SubjectDBAccess {
    ...
    public static function getListSubjectsFromAcademic($academicId, $term = false)
    {

        $params["academicId"] = $academicId;
        $entries = self::sqlAssignedSubjectsByGrade($params);
        ...
        if ($entries)
        {
            ...
            {
                if (ScheduleDBAccess::checkSubjectInSchedule($value->SUBJECT_ID, $academicId, $term))
                {
                    $obj = new stdClass();
                    $obj->OBJECT_ID = $value->SUBJECT_GRADE_ID;
                    $obj->SUBJECT_ID = $value->SUBJECT_ID;
                    $obj->SUBJECT_NAME = $value->SUBJECT_NAME;
                    $obj->SUBJECT_SHORT = $value->SUBJECT_SHORT;
    ...
    public static function sqlAssignedSubjectsByGrade($params)
    {
        ...
        $SQL = "";
        $SQL .= " SELECT ";
        $SQL .= " A.GUID AS GUID, A.SHORT AS SHORT, A.ID AS SUBJECT_ID";

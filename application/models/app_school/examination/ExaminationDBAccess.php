...
class ExaminationDBAccess {
    ...
    public static function findExamFromParentId($parentId) {
    ...
    public static function findExamFromSchoolYearId($schoolYearId) {

        $SQL = "SELECT DISTINCT *";
        $SQL .= " FROM t_examination";
        $SQL .= " WHERE";
        $SQL .= " SCHOOLYEAR_ID  = '" . $schoolYearId . "'";
        ...
        if ($facette) {
            switch ($facette->OBJECT_TYPE) {
                case "FOLDER":
                    ...
                    self::dbAccess()->delete('t_teacher_examination', array("EXAM_ID='" . $Id . "'"));

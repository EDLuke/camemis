...
class StudentAcademicDBAccess extends StudentDBAccess {
    ...
    public static function jsonTreeStudentsByClass($academicId)
    {
        ...
        if ($entries)
        {
            foreach ($entries as $key => $value)
            {
                ...
                $data[$key]['text'] = $value->SORTKEY . ") " . setShowText($value->LASTNAME . " " . $value->FIRSTNAME);
                $data[$key]['gender'] = "" . $value->GENDER . "";
                switch ($value->GENDER)
                {
                    case 1:
                        $data[$key]['iconCls'] = "icon-user_male";
                        break;
                    case 2:
                        $data[$key]['iconCls'] = "icon-user_female";
                        break;
                    default:
                        $data[$key]['iconCls'] = "icon-user_red";
                        break;
                }
                $data[$key]['mobilPhone'] = "" . $value->MOBIL_PHONE . "";
                $data[$key]['smsServices'] = "" . $value->SMS_SERVICES . "";
                $data[$key]['firstName'] = "" . $value->FIRSTNAME . "";
                $data[$key]['lastName'] = "" . $value->LASTNAME . "";
                $data[$key]['cls'] = "nodeTextBlue";
                $data[$key]['leaf'] = true;
            }
        }
    ...
    //
    //@Sea Peng
    public static function findStudentSchoolyear($studentId, $classId)
    {
    ...
    public static function checkStudentSchoolyearSubjectTerm($studentId, $academicId, $term, $subjectId)
    {
    ...
    public static function jsonActionEnrollStudentSubjectTerm($params)
    {
        $academicId = isset($params["academicId"]) ? addText($params["academicId"]) : "";
        ...
        $explode = explode("__", $field);
        $term = isset($explode[0]) ? $explode[0] : "";
        $subjectId = isset($explode[1]) ? $explode[1] : "";

        $academicObject = AcademicDBAccess::findGradeFromId($academicId);

        if ($academicObject && $subjectId && $term)
        {
            $CHECK = self::checkStudentSchoolyearSubjectTerm(
                            $studentId
                            , $academicId
                            , $term
                            , $subjectId);
            ...
            if ($comboValue)
            {
                ...
                {
                    $SAVEDATA['STUDENT_ID'] = $studentId;
                    $SAVEDATA['CLASS_ID'] = $academicObject->ID;
                    $SAVEDATA['CREDIT_ACADEMIC_ID'] = $academicObject->ID;
                    $SAVEDATA['CAMPUS_ID'] = $academicObject->CAMPUS_ID;
            ...
            {
                $WHERE = " CLASS_ID='" . $academicObject->ID . "'";
                $WHERE .= " AND STUDENT_ID='" . $studentId . "'";
                $WHERE .= " AND SUBJECT_ID='" . $subjectId . "'";
                $WHERE .= " AND TERM='" . $term . "'";
                $WHERE .= " AND SCHOOLYEAR_ID='" . $academicObject->SCHOOL_YEAR . "'";
                self::dbAccess()->delete("t_student_schoolyear_subject", $WHERE);
            }
        }

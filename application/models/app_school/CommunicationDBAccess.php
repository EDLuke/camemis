...
class CommunicationDBAccess {
            ...
            foreach ($result as $value) {
                ...
                switch ($value->SENDER_TYPE) {
                    case "STAFF":
                        ...
                        break;
                    case "STUDENT":
                        //$studentObject = StudentDBAccess::findStudentFromId($value->SENDER);
                        break;
                }
                //
    ...
    //@veasna
    public function jsonRemoveRecipientCommunication($params) {
        $selectionIds = $params["selectionIds"];
        ...
        if ($selectionIds != "") {
            ...
            if ($removeIds)
                foreach ($removeIds as $removeId) {
                    self::dbAccess()->delete('t_recipient_communication', array("COMMUNICATION_ID='" . $removeId . "'", "RECIPIENT_ID='" . Zend_Registry::get('USER')->ID . "'"));
                    $selectedCount++;
                }
        } else {
            $selectedCount = 0;
    ...
    public function jsonSaveCommunication($params) {
        ...
        if (isset($params["parentId"])) {
            ...
            $SAVEDATA['CONTENT'] = addText($newContent);
        } else {
        ...
        if (isset($params["communication_type"]))
            $SAVEDATA['COMMUNICATION_TYPE'] = isset($params["communication_type"]) ? addText($params["communication_type"]) : "";
